 ...
 */
import { pluginManifestV1Schema } from "@titanclip/shared";
import type { TitanClipPluginManifestV1 } from "@titanclip/shared";
import { PLUGIN_API_VERSION } from "@titanclip/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): TitanClipPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as TitanClipPluginManifestV1,
        };
      }
