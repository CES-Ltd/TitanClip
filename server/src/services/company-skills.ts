...
import { companySkills } from "@titanclip/db";
import { readTitanClipSkillSyncPreference, writeTitanClipSkillSyncPreference } from "@titanclip/adapter-utils/server-utils";
import type { TitanClipSkillEntry } from "@titanclip/adapter-utils/server-utils";
import type {
  CompanySkill,
...
import { normalizeAgentUrlKey } from "@titanclip/shared";
import { findServerAdapter } from "../adapters/index.js";
import { resolveTitanClipInstanceRoot } from "../home-paths.js";
...
function resolveManagedSkillsRoot(companyId: string) {
  return path.resolve(resolveTitanClipInstanceRoot(), "skills", companyId);
}
...
function deriveSkillSourceInfo(skill: CompanySkill): {
  ...
  if (metadata.sourceKind === "paperclip_bundled") {
    return {
      editable: false,
      editableReason: "Bundled TitanClip skills are read-only.",
      sourceLabel: "TitanClip bundled",
      sourceBadge: "titanclip",
      sourcePath: null,
    };
  }
  ...
  if (skill.sourceType === "local_path") {
    ...
    if (localSkillDir && localSkillDir.startsWith(managedRoot)) {
      return {
        editable: true,
        editableReason: null,
        sourceLabel: "TitanClip workspace",
        sourceBadge: "titanclip",
    ...
    for (const skill of skills) {
      ...
      out.push({
        ...
        requiredReason: required
          ? "Bundled TitanClip skills are always available for local adapters."
          : null,
      });
    }
    ...
    for (const agent of agentRows) {
      ...
      if (referencesSkill) {
        ...
        await agents.update(agent.id, {
          adapterConfig: writeTitanClipSkillSyncPreference(config, filtered),
        });
      }
    }
