...
import type { BillingType, ExecutionWorkspace, ExecutionWorkspaceConfig } from "@titanclip/shared";
import {
  agents,
  agentRuntimeState,
  agentTaskSessions,
...
import { trackAgentFirstHeartbeat } from "@titanclip/shared/telemetry";
...
export function heartbeatService(db: Db) {
  ...
  async function executeRun(runId: string) {
    ...
    if (runtimeServiceIntents.length > 0) {
      context.titanclipRuntimeServiceIntents = runtimeServiceIntents;
    } else {
      delete context.titanclipRuntimeServiceIntents;
    }
    ...
    try {
      ...
      if (runtimeServices.length > 0) {
        context.titanclipRuntimeServices = runtimeServices;
        context.titanclipRuntimePrimaryUrl =
      ...
      if (issueId && (executionWorkspace.created || runtimeServices.some((service) => !service.reused))) {
        try {
          ...
          await onLog(
            "stderr",
            `[titanclip] Failed to post workspace-ready comment: ${err instanceof Error ? err.message : String(err)}\n`,
          );
        }
      }
      ...
      if (adapterManagedRuntimeServices.length > 0) {
        ...
        context.titanclipRuntimeServices = combinedRuntimeServices;
        context.titanclipRuntimePrimaryUrl =
        ...
        if (issueId) {
          try {
            ...
            await onLog(
              "stderr",
              `[titanclip] Failed to post adapter-managed runtime comment: ${err instanceof Error ? err.message : String(err)}\n`,
            );
          }
        }
      }
