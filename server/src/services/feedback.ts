...
import { readTitanClipSkillSyncPreference } from "@titanclip/adapter-utils/server-utils";
import { claudeConfigDir, parseClaudeStreamJson } from "@titanclip/adapter-claude-local/server";
import { codexHomeDir, parseCodexJsonl } from "@titanclip/adapter-codex-local/server";
import { parseOpenCodeJsonl } from "@titanclip/adapter-opencode-local/server";
import {
...
import { resolveHomeAwarePath, resolveTitanClipInstanceRoot } from "../home-paths.js";
...
function captureStatusFromFiles(files: FeedbackTraceBundleFile[]): FeedbackTraceBundleCaptureStatus {
  ...
  const hasAdapterFiles = files.some((file) =>
    file.source !== "titanclip_run" &&
    file.source !== "titanclip_run_events" &&
    file.source !== "titanclip_run_log",
  );
...
async function buildOpenCodeTraceFiles(input: {
  ...
  const opencodeRoot = resolveHomeAwarePath(
    process.env.TITANCLIP_OPENCODE_STORAGE_DIR ?? "~/.local/share/opencode",
  );
...
async function buildAgentContext(
  ...
  const desiredSkillRefs = uniqueNonEmpty(readTitanClipSkillSyncPreference(adapterConfig).desiredSkills).slice(0, MAX_SKILLS);
...
async function buildFeedbackTraceBundleFromRow(
  ...
  let titanclipRun: Record<string, unknown> | null = null;
  ...
  if (!sourceRunId) {
    ...
    if (!run || run.companyId !== row.companyId) {
      ...
      titanclipRun = sanitizeFeedbackValue(
        {
          id: run.id,
      ...
      files.push(makeBundleFile({
        ...
        contentType: "application/json",
        source: "titanclip_run",
        contents: `${JSON.stringify(titanclipRun, null, 2)}\n`,
      }));

      const sanitizedEvents = sanitizeFeedbackValue(
        events,
        state,
        "bundle.titanclipRun.events",
        MAX_TRACE_FILE_CHARS,
      );
      files.push(makeBundleFile({
        path: "paperclip/run-events.json",
        contentType: "application/json",
        source: "titanclip_run_events",
      ...
      if (logText) {
        files.push(makeBundleFile({
          ...
          contentType: "application/x-ndjson",
          source: "titanclip_run_log",
          contents: `${sanitizeFeedbackText(logText, state, "bundle.titanclipRun.log", MAX_TRACE_FILE_CHARS)}\n`,
        }));
      } else {
