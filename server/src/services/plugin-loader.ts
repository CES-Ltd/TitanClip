 ...
 *    (`~/.titanclip/plugins/`) and `node_modules` for packages matching
...
/**
 * Naming convention for npm-published TitanClip plugins.
 * Packages matching this pattern are considered TitanClip plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
...
export interface DiscoveredPlugin {
  ...
  manifest: TitanClipPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.titanclip/plugins/ local directory
...
function getDeclaredPageRoutePaths(manifest: TitanClipPluginManifestV1): string[] {
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: TitanClipPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover TitanClip plugins installed as npm packages in the current
   ...
   *
   * Returns null if the package is not a TitanClip plugin.
   * Throws if the package is a TitanClip plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<TitanClipPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: TitanClipPluginManifestV1;
    newManifest: TitanClipPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the TitanClip plugin naming convention.
...
export function isPluginPackageName(name: string): boolean {
  ...
  // Also accept scoped packages like @acme/plugin-linear or @titanclip/plugin-*
  if (name.includes("/")) {
...
function getMinimumHostVersion(manifest: TitanClipPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumTitanClipVersion;
}
...
export function getPluginUiContributionMetadata(
  manifest: TitanClipPluginManifestV1,
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: TitanClipPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as TitanClipPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a TitanClip plugin (no manifest found).`,
      );
    }
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<TitanClipPluginManifestV1> {
    let raw: unknown;
   ...
   * if the package is not a TitanClip plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasTitanClipPlugin = "paperclipPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasTitanClipPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<TitanClipPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: TitanClipPluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @titanclip/shared exports). Run those workers through
