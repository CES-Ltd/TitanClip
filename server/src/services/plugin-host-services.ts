...
import { pluginLogs, agentTaskSessions as agentTaskSessionsTable } from "@titanclip/db";
 ...
 *
 * This implementation delegates to the core TitanClip domain services,
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@titanclip/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
