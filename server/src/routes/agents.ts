...
import { agents as agentsTable, companies, heartbeatRuns, issues as issuesTable } from "@titanclip/db";
...
import { trackAgentCreated } from "@titanclip/shared/telemetry";
...
import { runClaudeLogin } from "@titanclip/adapter-claude-local/server";
import {
...
import { ensureOpenCodeModelConfiguredAndAvailable } from "@titanclip/adapter-opencode-local/server";
import {
...
export function agentRoutes(db: Db) {
  ...
  async function buildRuntimeSkillConfig(
    ...
    return {
      ...config,
      titanclipRuntimeSkills: runtimeSkillEntries,
    };
  }
  ...
  async function resolveDesiredSkillAssignment(
    ...
    return {
      adapterConfig: writeTitanClipSkillSyncPreference(adapterConfig, desiredSkills),
      desiredSkills,
      runtimeSkillEntries,
    };
  }
  ...
  router.get("/agents/:id/skills", async (req, res) => {
    ...
    if (!adapter?.listSkills) {
      const preference = readTitanClipSkillSyncPreference(
  ...
  router.post("/companies/:companyId/agent-hires", validate(createAgentHireSchema), async (req, res) => {
    ...
    const adminSettings = await instanceSettings.getAdmin();
    if (adminSettings.allowedAdapterTypes !== null) {
      const requestedAdapter = req.body.adapterType;
      if (requestedAdapter && !adminSettings.allowedAdapterTypes.includes(requestedAdapter)) {
        throw badRequest(`Adapter type "${requestedAdapter}" is not allowed by admin policy.`);
      }
    }
    if (adminSettings.allowedRoles !== null) {
      const requestedRole = req.body.role ?? "general";
      if (!adminSettings.allowedRoles.includes(requestedRole)) {
        throw badRequest(`Agent role "${requestedRole}" is not allowed by admin policy.`);
      }
    }
    if (adminSettings.allowedModelsPerAdapter !== null) {
      const requestedAdapter = req.body.adapterType;
      const modelAllowlist = requestedAdapter ? adminSettings.allowedModelsPerAdapter[requestedAdapter] : null;
      if (modelAllowlist !== null && modelAllowlist !== undefined) {
        const requestedModel = (req.body.adapterConfig as Record<string, unknown> | undefined)?.model as string | undefined;
        if (requestedModel && !modelAllowlist.includes(requestedModel)) {
          throw badRequest(`Model "${requestedModel}" is not allowed for adapter "${requestedAdapter}" by admin policy.`);
        }
      }
    }
  ...
  // --- Chat endpoint: invokes agent's configured adapter directly ---
  router.post("/companies/:companyId/agents/:agentId/chat", async (req, res) => {
    ...
    const agent = await svc.getById(agentId as string);
    if (!agent) { res.status(404).json({ error: "Agent not found" }); return; }
    assertCompanyAccess(req, agent.companyId);

    // Resolve adapter config with secrets
    const rawConfig = (agent.adapterConfig ?? {}) as Record<string, unknown>;
    const resolved = await secretsSvc.resolveAdapterConfigForRuntime(agent.companyId, rawConfig);
    const adapterConfig = resolved.config;
    ...
    // Get the server adapter for this agent's type
    const { getServerAdapter } = await import("../adapters/index.js");
    let adapter;
    try {
      adapter = getServerAdapter(agent.adapterType);
    } catch {
      res.status(400).json({ error: `Adapter "${agent.adapterType}" is not available on this server.` });
      return;
    }

    // Collect output from the adapter
    ...
    try {
      ...
      // Extract the response text from adapter result
      let reply = "";

      // Try resultJson first (structured output from adapter)
      if (result.resultJson) {
        const rj = result.resultJson as Record<string, unknown>;
        reply = (rj.result as string) ?? (rj.text as string) ?? (rj.response as string) ?? "";
      }

      // If no structured result, try summary
      if (!reply && result.summary) {
        reply = result.summary;
      }

      // If still nothing, use collected stdout (filtered for non-JSON content)
      if (!reply) {
        reply = outputChunks
      ...
      // Last resort
      if (!reply) {
        reply = "I processed your message but didn't generate a text response. The adapter may require a different interaction pattern.";
      }
      ...
      res.status(500).json({
        error: `Chat via ${agent.adapterType} adapter failed: ${errorMsg}`,
      });
    }
  });
