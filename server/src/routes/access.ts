...
/** Resolve the TitanClip repo skills directory (built-in / managed skills). */
function resolveTitanClipSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const paperclipSkillsDir = resolveTitanClipSkillsDir();

  // Build set of TitanClip-managed skill names
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isTitanClipManaged: paperclipSkillNames.has(entry.name),
      });
    }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.paperclipApiUrl)) {
    const legacyTitanClipApiUrl = nonEmptyTrimmedString(input.paperclipApiUrl);
    if (legacyTitanClipApiUrl) merged.paperclipApiUrl = legacyTitanClipApiUrl;
  }
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawTitanClipApiUrl =
    typeof defaults.paperclipApiUrl === "string"
      ? defaults.paperclipApiUrl.trim()
      : "";
  if (rawTitanClipApiUrl) {
    try {
      const parsedTitanClipApiUrl = new URL(rawTitanClipApiUrl);
      if (
        parsedTitanClipApiUrl.protocol !== "http:" &&
        parsedTitanClipApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_paperclip_api_url_protocol",
          level: "warn",
          message: `paperclipApiUrl must use http:// or https:// (got ${parsedTitanClipApiUrl.protocol}).`
        });
      } else {
        normalized.paperclipApiUrl = parsedTitanClipApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_paperclip_api_url_configured",
          level: "info",
          message: `paperclipApiUrl set to ${parsedTitanClipApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        ...
        level: "warn",
        message: `Invalid paperclipApiUrl: ${rawTitanClipApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your TitanClip host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "TitanClip is bound to loopback in authenticated/private mode.",
...
function buildInviteOnboardingManifest(
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/paperclip-claimed-api-key.json and load TITANCLIP_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the TitanClip hostname is reachable and allowed via `pnpm paperclipai allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this TitanClip API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # TitanClip OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, TitanClip generates and persists one during join so pairing approvals are stable.
    ...
    The board approves the join request in TitanClip before key claim is allowed.
    ...
    And set the TITANCLIP_API_KEY and TITANCLIP_API_URL in your environment variables as specified here:
    ...
    Then set TITANCLIP_API_KEY and TITANCLIP_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install TitanClip skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your TITANCLIP_API_URL to the top of your skill and note the path to your TITANCLIP_API_URL
    ...
    ${
      ...
      "Ensure TitanClip is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested TitanClip base URLs to try");
    ...
    appendBlock(`
      ...
      - then restart TitanClip and retry onboarding.
    `);
  }
...
export function accessRoutes(
  ...
  router.get("/skills/index", (_req, res) => {
    res.json({
      skills: [
        { name: "titanclip", path: "/api/skills/paperclip" },
        {
