const FAVICON_BLOCK_START = "<!-- TITANCLIP_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- TITANCLIP_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- TITANCLIP_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- TITANCLIP_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.TITANCLIP_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.TITANCLIP_WORKTREE_NAME) ?? nonEmpty(env.TITANCLIP_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.TITANCLIP_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
