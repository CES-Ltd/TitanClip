...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".titanclip", PAPERCLIP_CONFIG_BASENAME);
...
export function resolveTitanClipEnvPath(overrideConfigPath?: string): string {
  return path.resolve(path.dirname(resolveTitanClipConfigPath(overrideConfigPath)), PAPERCLIP_ENV_FILENAME);
}
