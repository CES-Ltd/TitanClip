...
import { execute } from "@titanclip/adapter-codex-local/server";
...
describe("codex execute", () => {
  it("uses a TitanClip-managed CODEX_HOME outside worktree mode while preserving shared auth and config", async () => {
    ...
    try {
      ...
      expect(logs).toContainEqual(
        expect.objectContaining({
          stream: "stdout",
          chunk: expect.stringContaining("Using TitanClip-managed Codex home"),
        }),
      );
    } finally {
  ...
  it("emits a command note that Codex auto-applies repo-scoped AGENTS.md files", async () => {
    ...
    try {
      ...
      expect(commandNotes).toContain(
        "Codex exec automatically applies repo-scoped AGENTS.md instructions from the current workspace; TitanClip does not currently suppress that discovery.",
      );
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
