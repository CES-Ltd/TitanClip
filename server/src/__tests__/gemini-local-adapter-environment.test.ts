...
import { testEnvironment } from "@titanclip/adapter-gemini-local/server";
...
const outPath = process.env.TITANCLIP_TEST_ARGS_PATH;
if (outPath) {
    ...
    const result = await testEnvironment({
      ...
      config: {
        ...
        env: {
          ...
          TITANCLIP_TEST_ARGS_PATH: argsCapturePath,
