...
import type { CompanyPortabilityFileEntry } from "@titanclip/shared";
...
describe("company portability", () => {
    ...
    const companySkills = [
      {
        ...
        slug: "paperclip",
        name: "paperclip",
        description: "TitanClip coordination skill",
        markdown: "---\nname: paperclip\ndescription: TitanClip coordination skill\n---\n\n# TitanClip\n",
        sourceType: "github",
  ...
  it("exports referenced skills as stubs by default with sanitized TitanClip extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "TitanClip"');
  ...
  it("exports default sidebar order into the TitanClip extension and manifest", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/paperclipai/paperclip/paperclip/SKILL.md"])).toContain("# TitanClip");
    expect(asTextFile(exported.files["skills/paperclipai/paperclip/paperclip/SKILL.md"])).toContain("metadata:");
  ...
  it("exports portable project workspace metadata and remaps it on import", async () => {
    ...
    await portability.importBundle({
      ...
      target: {
        mode: "new_company",
        newCompanyName: "Imported TitanClip",
      },
  ...
  it("exports routines as recurring task packages with TitanClip routine extensions", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("imports recurring task packages as routines instead of one-time issues", async () => {
    ...
    const preview = await portability.previewImport({
      ...
      target: { mode: "new_company", newCompanyName: "Imported TitanClip" },
      agents: "all",
      collisionStrategy: "rename",
    });
  ...
  it("migrates legacy schedule.recurrence imports into routine triggers", async () => {
    ...
    const files = {
      "COMPANY.md": ['---', 'schema: "agentcompanies/v1"', 'name: "Imported TitanClip"', "---", ""].join("\n"),
  ...
  it("imports a vendor-neutral package without .paperclip.yaml", async () => {
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported TitanClip");
