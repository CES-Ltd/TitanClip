...
import { ensureCodexSkillsInjected } from "@titanclip/adapter-codex-local/server";

async function makeTempDir(prefix: string): Promise<string> {
  return fs.mkdtemp(path.join(os.tmpdir(), prefix));
}

async function createTitanClipRepoSkill(root: string, skillName: string) {
...
describe("codex local adapter skill injection", () => {
  ...
  it("repairs a Codex TitanClip skill symlink that still points at another live checkout", async () => {
    ...
    await createTitanClipRepoSkill(currentRepo, "paperclip");
    await createTitanClipRepoSkill(oldRepo, "paperclip");
  ...
  it("preserves a custom Codex skill symlink outside TitanClip repo checkouts", async () => {
    const currentRepo = await makeTempDir("paperclip-codex-current-");
  ...
  it("prunes broken symlinks for unavailable TitanClip repo skills before Codex starts", async () => {
    const currentRepo = await makeTempDir("paperclip-codex-current-");
    const oldRepo = await makeTempDir("paperclip-codex-old-");
    const skillsHome = await makeTempDir("paperclip-codex-home-");
    cleanupDirs.add(currentRepo);
    ...
    await createTitanClipRepoSkill(currentRepo, "paperclip");
    await createTitanClipRepoSkill(oldRepo, "agent-browser");
  ...
  it("preserves other live TitanClip skill symlinks in the shared workspace skill directory", async () => {
    const currentRepo = await makeTempDir("paperclip-codex-current-");
    const skillsHome = await makeTempDir("paperclip-codex-home-");
    cleanupDirs.add(currentRepo);
    cleanupDirs.add(skillsHome);
    ...
    await createTitanClipRepoSkill(currentRepo, "paperclip");
    await createTitanClipRepoSkill(currentRepo, "agent-browser");
    await fs.symlink(
