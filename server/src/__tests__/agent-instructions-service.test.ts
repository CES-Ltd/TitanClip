...
describe("agent instructions service", () => {
  const originalTitanClipHome = process.env.TITANCLIP_HOME;
  const originalTitanClipInstanceId = process.env.TITANCLIP_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalTitanClipHome === undefined) delete process.env.TITANCLIP_HOME;
    else process.env.TITANCLIP_HOME = originalTitanClipHome;
    if (originalTitanClipInstanceId === undefined) delete process.env.TITANCLIP_INSTANCE_ID;
    else process.env.TITANCLIP_INSTANCE_ID = originalTitanClipInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    ...
    process.env.TITANCLIP_INSTANCE_ID = "test-instance";
