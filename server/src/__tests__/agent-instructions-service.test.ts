...
describe("agent instructions service", () => {
  const originalTitanClipHome = process.env.PAPERCLIP_HOME;
  const originalTitanClipInstanceId = process.env.PAPERCLIP_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalTitanClipHome === undefined) delete process.env.PAPERCLIP_HOME;
    else process.env.PAPERCLIP_HOME = originalTitanClipHome;
    if (originalTitanClipInstanceId === undefined) delete process.env.PAPERCLIP_INSTANCE_ID;
    else process.env.PAPERCLIP_INSTANCE_ID = originalTitanClipInstanceId;
