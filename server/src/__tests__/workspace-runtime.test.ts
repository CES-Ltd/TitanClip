...
import { resolveTitanClipConfigPath } from "../paths.ts";
import type { WorkspaceOperation } from "@titanclip/shared";
...
async function createTempRepo(defaultBranch = "main") {
  ...
  await runGit(repoRoot, ["config", "user.name", "TitanClip Test"]);
...
describe("sanitizeRuntimeServiceBaseEnv", () => {
  it("removes inherited TitanClip and pnpm auth flags before spawning runtime services", () => {
...
describe("realizeExecutionWorkspace", () => {
  ...
  it("writes an isolated repo-local TitanClip config and worktree branding when provisioning", async () => {
    const repoRoot = await createTempRepo();
...
describe("ensureRuntimeServicesForRun", () => {
  ...
  it("does not leak parent TitanClip instance env into runtime service commands", async () => {
