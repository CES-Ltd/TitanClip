...
export async function startServer(): Promise<StartedServer> {
  ...
  const LOCAL_BOARD_USER_EMAIL = "local@titanclip.local";
  ...
  if (config.databaseUrl) {
    ...
    if (runningPid) {
      ...
      const configuredAdminConnectionString = `postgres://titanclip:titanclip@127.0.0.1:${configuredPort}/postgres`;
      try {
        ...
        await ensurePostgresDatabase(configuredAdminConnectionString, "titanclip");
        logger.warn(
    ...
    const embeddedAdminConnectionString = `postgres://titanclip:titanclip@127.0.0.1:${port}/postgres`;
    const dbStatus = await ensurePostgresDatabase(embeddedAdminConnectionString, "titanclip");
    if (dbStatus === "created") {
      logger.info("Created embedded PostgreSQL database: titanclip");
    }
  
    const embeddedConnectionString = `postgres://titanclip:titanclip@127.0.0.1:${port}/titanclip`;
...
if (isMainModule(import.meta.url)) {
  void startServer().catch((err) => {
    logger.error({ err }, "TitanClip server failed to start");
    process.exit(1);
  });
}
