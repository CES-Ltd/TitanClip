...
import { resolveTitanClipConfigPath, resolveTitanClipEnvPath } from "./paths.js";
...
function resolveAgentJwtSecretStatus(
  ...
  const envValue = process.env.TITANCLIP_AGENT_JWT_SECRET?.trim();
  if (envValue) {
    return {
      status: "pass",
      message: "set",
  ...
  if (existsSync(envFilePath)) {
    ...
    const fileValue = typeof parsed.TITANCLIP_AGENT_JWT_SECRET === "string" ? parsed.TITANCLIP_AGENT_JWT_SECRET.trim() : "";
    if (fileValue) {
      return {
        status: "warn",
...
export function printStartupBanner(opts: StartupBannerOptions): void {
  ...
  const envFilePath = resolveTitanClipEnvPath();
