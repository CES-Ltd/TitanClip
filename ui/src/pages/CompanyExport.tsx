...
/**
 * Filter .titanclip.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterTitanClipYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Company](https://titanclip.app) package.");
  lines.push("");
  ...
  lines.push("```");
  lines.push("");
  lines.push("See [TitanClip](https://titanclip.app) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [TitanClip](https://titanclip.app) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
export function CompanyExport() {
  ...
  // Recompute .titanclip.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .titanclip.yaml
    const yamlPath = exportData.titanclipExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterTitanClipYaml(exportData.files[yamlPath], checkedFiles);
    }
