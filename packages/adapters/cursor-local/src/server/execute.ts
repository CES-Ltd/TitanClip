...
export async function ensureCursorSkillsInjected(
  ...
  const skillsEntries = options.skillsEntries
    ?? (options.skillsDir
      ...
      : await readTitanClipRuntimeSkillEntries({}, __moduleDir));
  ...
  for (const entry of skillsEntries) {
    ...
    try {
      const result = await ensureTitanClipSkillSymlink(entry.source, target, linkSkill);
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const cursorSkillEntries = await readTitanClipRuntimeSkillEntries(config, __moduleDir);
  const desiredCursorSkillNames = resolveTitanClipDesiredSkillNames(config, cursorSkillEntries);
