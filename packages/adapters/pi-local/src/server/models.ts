...
import type { AdapterModel } from "@titanclip/adapter-utils";
import { asString, runChildProcess } from "@titanclip/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.TITANCLIP_PI_COMMAND === "string" &&
    process.env.TITANCLIP_PI_COMMAND.trim().length > 0
      ? process.env.TITANCLIP_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["TITANCLIP_", "npm_", "NPM_"] as const;
