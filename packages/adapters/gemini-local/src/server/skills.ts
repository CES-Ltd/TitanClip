...
async function buildGeminiSkillSnapshot(config: Record<string, unknown>): Promise<AdapterSkillSnapshot> {
  const availableEntries = await readTitanClipRuntimeSkillEntries(config, __moduleDir);
  const desiredSkills = resolveTitanClipDesiredSkillNames(config, availableEntries);
...
export async function syncGeminiSkills(
  ...
  const availableEntries = await readTitanClipRuntimeSkillEntries(ctx.config, __moduleDir);
  ...
  for (const available of availableEntries) {
    ...
    await ensureTitanClipSkillSymlink(available.source, target);
  }
...
export function resolveGeminiDesiredSkillNames(
  ...
  return resolveTitanClipDesiredSkillNames(config, availableEntries);
}
