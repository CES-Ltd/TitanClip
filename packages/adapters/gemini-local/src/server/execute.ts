...
import {
  ...
  ensureTitanClipSkillSymlink,
  joinPromptSections,
  ensurePathInEnv,
  readTitanClipRuntimeSkillEntries,
  resolveCommandForLogs,
  resolveTitanClipDesiredSkillNames,
...
function renderPaperclipEnvNote(env: Record<string, string>): string {
  ...
  return [
    ...
    `The following TITANCLIP_* environment variables are available in this run: ${paperclipKeys.join(", ")}`,
...
function renderApiAccessNote(env: Record<string, string>): string {
  if (!hasNonEmptyEnvValue(env, "TITANCLIP_API_URL") || !hasNonEmptyEnvValue(env, "TITANCLIP_API_KEY")) return "";
  return [
    "Paperclip API access note:",
    "Use run_shell_command with curl to make Paperclip API requests.",
    "GET example:",
    `  run_shell_command({ command: "curl -s -H \\"Authorization: Bearer $TITANCLIP_API_KEY\\" \\"$TITANCLIP_API_URL/api/agents/me\\"" })`,
    "POST/PATCH example:",
    `  run_shell_command({ command: "curl -s -X POST -H \\"Authorization: Bearer $TITANCLIP_API_KEY\\" -H 'Content-Type: application/json' -H \\"X-Paperclip-Run-Id: $TITANCLIP_RUN_ID\\" -d '{...}' \\"$TITANCLIP_API_URL/api/issues/{id}/checkout\\"" })`,
    "",
    "",
  ].join("\n");
}
...
async function ensureGeminiSkillsInjected(
  ...
  for (const entry of selectedEntries) {
    ...
    try {
      const result = await ensureTitanClipSkillSymlink(entry.source, target);
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const geminiSkillEntries = await readTitanClipRuntimeSkillEntries(config, __moduleDir);
  const desiredGeminiSkillNames = resolveTitanClipDesiredSkillNames(config, geminiSkillEntries);
  ...
  const linkedIssueIds = Array.isArray(context.issueIds)
    ...
    : [];
  if (wakeTaskId) env.TITANCLIP_TASK_ID = wakeTaskId;
  if (wakeReason) env.TITANCLIP_WAKE_REASON = wakeReason;
  if (wakeCommentId) env.TITANCLIP_WAKE_COMMENT_ID = wakeCommentId;
  if (approvalId) env.TITANCLIP_APPROVAL_ID = approvalId;
  if (approvalStatus) env.TITANCLIP_APPROVAL_STATUS = approvalStatus;
  if (linkedIssueIds.length > 0) env.TITANCLIP_LINKED_ISSUE_IDS = linkedIssueIds.join(",");
  if (effectiveWorkspaceCwd) env.TITANCLIP_WORKSPACE_CWD = effectiveWorkspaceCwd;
  if (workspaceSource) env.TITANCLIP_WORKSPACE_SOURCE = workspaceSource;
  if (workspaceId) env.TITANCLIP_WORKSPACE_ID = workspaceId;
  if (workspaceRepoUrl) env.TITANCLIP_WORKSPACE_REPO_URL = workspaceRepoUrl;
  if (workspaceRepoRef) env.TITANCLIP_WORKSPACE_REPO_REF = workspaceRepoRef;
  if (agentHome) env.AGENT_HOME = agentHome;
  if (workspaceHints.length > 0) env.TITANCLIP_WORKSPACES_JSON = JSON.stringify(workspaceHints);
