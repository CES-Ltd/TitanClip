...
async function pruneBrokenUnavailableTitanClipSkillSymlinks(
  skillsHome: string,
...
export async function ensureCodexSkillsInjected(
  ...
  const allSkillsEntries = options.skillsEntries ?? await readTitanClipRuntimeSkillEntries({}, __moduleDir);
  ...
  await pruneBrokenUnavailableTitanClipSkillSymlinks(
    skillsHome,
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const configuredCodexHome =
    typeof envConfig.CODEX_HOME === "string" && envConfig.CODEX_HOME.trim().length > 0
      ...
      : null;
  const codexSkillEntries = await readTitanClipRuntimeSkillEntries(config, __moduleDir);
