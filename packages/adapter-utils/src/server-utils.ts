...
const TITANCLIP_SKILL_ROOT_RELATIVE_CANDIDATES = [
  "../../skills",
  "../../../../../skills",
];

export interface TitanClipSkillEntry {
  key: string;
  runtimeName: string;
  source: string;
  required?: boolean;
...
interface PersistentSkillSnapshotOptions {
  adapterType: string;
  availableEntries: TitanClipSkillEntry[];
...
> {
  if (entry.required) {
    return {
      ...
      originLabel: "Required by TitanClip",
      readOnly: false,
    };
  }
  return {
    origin: "company_managed",
    originLabel: "Managed by TitanClip",
    readOnly: false,
  };
}
...
export function buildInvocationEnvForLogs(
  ...
  if (resolvedCommand) {
    merged[options.resolvedCommandEnvKey ?? "TITANCLIP_RESOLVED_COMMAND"] = resolvedCommand;
  }

  return redactEnvForLogs(merged);
}

export function buildTitanClipEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const vars: Record<string, string> = {
    TITANCLIP_AGENT_ID: agent.id,
    TITANCLIP_COMPANY_ID: agent.companyId,
  };
  const runtimeHost = resolveHostForUrl(
    process.env.TITANCLIP_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  const runtimePort = process.env.TITANCLIP_LISTEN_PORT ?? process.env.PORT ?? "3100";
  const apiUrl = process.env.TITANCLIP_API_URL ?? `http://${runtimeHost}:${runtimePort}`;
  vars.TITANCLIP_API_URL = apiUrl;
  return vars;
}
...
export async function resolveTitanClipSkillsDir(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<string | null> {
  const candidates = [
    ...TITANCLIP_SKILL_ROOT_RELATIVE_CANDIDATES.map((relativePath) => path.resolve(moduleDir, relativePath)),
...
export async function listTitanClipSkillEntries(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<TitanClipSkillEntry[]> {
  const root = await resolveTitanClipSkillsDir(moduleDir, additionalCandidates);
  if (!root) return [];
  ...
  try {
    ...
    return entries
      ...
      .map((entry) => ({
        key: `titanclip/${entry.name}`,
        runtimeName: entry.name,
        source: path.join(root, entry.name),
        required: true,
        requiredReason: "Bundled TitanClip skills are always available for local adapters.",
      }));
  } catch {
    return [];
  }
...
export function buildPersistentSkillSnapshot(
  ...
  for (const desiredSkill of desiredSkills) {
    ...
    warnings.push(`Desired skill "${desiredSkill}" is not available from the TitanClip skills directory.`);
    entries.push({
      key: desiredSkill,
      runtimeName: null,
      desired: true,
      ...
      sourcePath: null,
      targetPath: null,
      detail: "TitanClip cannot find this skill in the local runtime skills directory.",
...
function normalizeConfiguredPaperclipRuntimeSkills(value: unknown): TitanClipSkillEntry[] {
  if (!Array.isArray(value)) return [];
  const out: TitanClipSkillEntry[] = [];
...
export async function readTitanClipRuntimeSkillEntries(
  ...
  return listTitanClipSkillEntries(moduleDir, additionalCandidates);
}

export async function readTitanClipSkillMarkdown(
  moduleDir: string,
  skillKey: string,
): Promise<string | null> {
  ...
  const entries = await listTitanClipSkillEntries(moduleDir);
...
export function readTitanClipSkillSyncPreference(config: Record<string, unknown>): {
  explicit: boolean;
  desiredSkills: string[];
} {
...
function canonicalizeDesiredTitanClipSkillReference(
  reference: string,
...
export function resolveTitanClipDesiredSkillNames(
  config: Record<string, unknown>,
  availableEntries: Array<{ key: string; runtimeName?: string | null; required?: boolean }>,
): string[] {
  const preference = readTitanClipSkillSyncPreference(config);
  ...
  const desiredSkills = preference.desiredSkills
    .map((reference) => canonicalizeDesiredTitanClipSkillReference(reference, availableEntries))
    .filter(Boolean);
  return Array.from(new Set([...requiredSkills, ...desiredSkills]));
}

export function writeTitanClipSkillSyncPreference(
  config: Record<string, unknown>,
  desiredSkills: string[],
...
export async function ensureTitanClipSkillSymlink(
  source: string,
  target: string,
