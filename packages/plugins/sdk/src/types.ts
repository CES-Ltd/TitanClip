/**
 * Core types for the TitanClip plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@titanclip/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @titanclip/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of TitanClip object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the TitanClip UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit TitanClip domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core TitanClip domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the TitanClip secret provider
...
export interface PluginSecretsClient {
   ...
   * in the TitanClip secret provider (e.g. `"MY_API_KEY"`).
   *
 ...
 * ```
 *
 * @see usePluginStream in `@titanclip/plugin-sdk/ui`
 */
 ...
 * @example
 * ```ts
 * import { definePlugin } from "@titanclip/plugin-sdk";
 *
