...
import type { TitanClipPluginManifestV1 } from "@titanclip/shared";

import type { TitanClipPlugin } from "./define-plugin.js";
import type {
 ...
 * import { startWorkerRpcHost } from "@titanclip/plugin-sdk";
 *
 * startWorkerRpcHost({ plugin });
 * ```
 *
...
export function startWorkerRpcHost(options: WorkerRpcHostOptions): WorkerRpcHost {
  ...
  let manifest: TitanClipPluginManifestV1 | null = null;
  ...
  const toolHandlers = new Map<string, {
    declaration: Pick<import("@titanclip/shared").PluginToolDeclaration, "displayName" | "description" | "parametersSchema">;
