# @paperclipai/create-titanclip-plugin

Scaffolding tool for creating new Paperclip plugins.

```bash
npx @paperclipai/create-titanclip-plugin my-plugin
```

Or with options:

```bash
npx @paperclipai/create-titanclip-plugin @acme/my-plugin \
  --template connector \
  --category connector \
...
- dev server script for hot-reload (`titanclip-plugin-dev-server`)
