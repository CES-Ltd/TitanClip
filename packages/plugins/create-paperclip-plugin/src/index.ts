...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "titanclip-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@titanclip/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@titanclip/plugin-sdk": sdkDependency,
...
import { createPluginBundlerPresets } from "@titanclip/plugin-sdk/bundlers";
...
`,
  ...
  writeFile(
    ...
    `import type { TitanClipPluginManifestV1 } from "@titanclip/plugin-sdk";

const manifest: TitanClipPluginManifestV1 = {
  id: ${quote(manifestId)},
  apiVersion: 1,
  version: "0.1.0",
...
`,
  ...
  writeFile(
    ...
    `import { definePlugin, runWorker } from "@titanclip/plugin-sdk";
...
`,
  ...
  writeFile(
    ...
    `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@titanclip/plugin-sdk/ui";
...
import { createTestHarness } from "@titanclip/plugin-sdk/testing";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@titanclip/plugin-sdk\` and \`@titanclip/shared\` from a local Paperclip checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.paperclip-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
- \`pnpm build\` uses esbuild presets from \`@titanclip/plugin-sdk/bundlers\`.
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-titanclip-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <paperclip-sdk-path>]");
    process.exit(1);
  }
