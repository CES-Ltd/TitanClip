...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://titanclip:titanclip@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "titanclip");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://titanclip:titanclip@127.0.0.1:${preferredPort}/titanclip`,
  ...
  if (runningPid) {
    ...
    const adminConnectionString = `postgres://titanclip:titanclip@127.0.0.1:${port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "titanclip");
    return {
      connectionString: `postgres://titanclip:titanclip@127.0.0.1:${port}/titanclip`,
  ...
  const adminConnectionString = `postgres://titanclip:titanclip@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "titanclip");

  return {
    connectionString: `postgres://titanclip:titanclip@127.0.0.1:${selectedPort}/titanclip`,
