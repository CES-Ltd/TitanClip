...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".titanclip", CONFIG_BASENAME);
...
function resolveTitanClipConfigPath(): string {
  if (process.env.TITANCLIP_CONFIG?.trim()) {
    return path.resolve(process.env.TITANCLIP_CONFIG.trim());
  }
