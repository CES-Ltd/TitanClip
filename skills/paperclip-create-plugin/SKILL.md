---
name: titanclip-create-plugin
description: >
  Create new TitanClip plugins with the current alpha SDK/runtime. Use when
...
# Create a TitanClip Plugin

Use this skill when the task is to create, scaffold, or document a TitanClip plugin.
...
node packages/plugins/create-titanclip-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the TitanClip repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.titanclip-sdk/`:

```bash
pnpm --filter @titanclipai/create-titanclip-plugin build
node packages/plugins/create-titanclip-plugin/dist/index.js @acme/plugin-name \
...
- is installed into TitanClip from an absolute local path during development
